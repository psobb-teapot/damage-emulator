...
ソースの正本は [rappyruns](https://github.com/PSOBBAITools/rappyruns) (非公開リポジトリ) の
...
- **全123武器** (ATP範囲・ATA・最大グラインド・Hit%/属性上限・特殊・段ごとのヒット数・射程・
  装備可能クラス — wiki.pioneer2.net の12ビットフラグを一括取得)
...
- **バフ・装備**: 折りたたみパネル + 有効な設定をチップ表示 (シフタ30・V501 等)。
  セット効果の発動を明示
- **表示はタブで切替** (設定エリアは常に上部、結果系は一度に1つ):
  - **単体詳細**: 計算根拠 (ATA合計・実効ATP・実効DFP)、敵HPバーへの期待ダメージ積み上げ
    (色=攻撃タイプ、濃さ=命中率)、キル確率 (クリティカル依存の内訳付き)、
    ヒット表 (距離ペナルティ対象時は命中率レンジ)
  - **複数の敵** (psostats 相当): 種族別一括追加・全追加で敵をリストへ入れ、
    合計ダメージ・%HP・キル確率・総合命中率を一覧。列ソート、行クリックで単体詳細へ
  - **確定ライン (逆算)**: 「想定 Hit%」スライダー (武器の Hit% とは独立) を基準に、
    命中100%に必要な Hit% (コンボ段ごと) と確定撃破ヒット数
    (最小ロール・クリなし。多段ヒット武器は ×1〜×n の ✓/△/× 判定) を逆算
  - **クラス比較**: 現在の武器・装備・敵に対する全12クラスの最適コンボを一覧し、
    行クリックで反映
...
- キル確率の計算では、1ヒットのダメージを [min, max] の整数一様分布とみなし
  (武器ATP・キャラATPの乱数ロールの近似)、命中/クリティカル/特殊発動の分岐とともに
  残りHPの確率分布へ畳み込む。クリティカルは各ロール値に ×1.5 切り捨てを適用。
  幅が非常に広い場合は等確率バケット (最大128点) で近似する (誤差 ≤ 0.4%)。
