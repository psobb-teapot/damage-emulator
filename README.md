...
- **複数の敵で比較** (psostats 相当): 種族別一括追加・全追加ボタンで敵を比較リストへ入れ、
  同じキャラ/装備/コンボでの合計ダメージ・%HP・キル確率・総合命中率を一覧表示。
  列ヘッダでソート、行クリックで詳細表示へ切替
- **確定ライン (逆算) モード**: 比較テーブルを切り替えると、各敵について
  「命中100%に必要な Hit%」(コンボ段ごと) と「確定撃破に必要なヒット数」
  (最小ロール・クリなし。Dark Flow 等の多段ヒットは ×1〜×n で ✓/△/× 表示) を逆算。
  Hit% を変えると達成状況が即時に色分けされる
- **全クラスで比較**: 現在の武器・装備・敵に対し全12クラスの最適コンボ
  (確定ダメージ/キル確率/総合命中/フレーム) を一覧し、行クリックで反映
